...
const steps = ["Estudiante", "Mi semana", "Pago", "Confirmación"];

export function PortalShell({ step, children }: { step: number; children: React.ReactNode }) {
  return (
    <div className="flex min-h-dvh flex-col pb-[max(1rem,env(safe-area-inset-bottom))]">
      ...
      <div className="border-b border-[var(--line)] bg-[var(--paper)] px-4 sm:px-6">
        <ol className="mx-auto flex max-w-2xl items-center justify-between py-2.5" aria-label="Progreso de compra">
                ...
                <span
                  className={`grid size-6 place-items-center rounded-full border text-[0.65rem] tabular-nums ${
                    active
                      ...
                      : completed
                        ? "border-[var(--pine)] bg-[var(--pine-soft)] text-[var(--pine-dark)]"
                        : "border-[var(--line)] text-[var(--muted)]"
                  }`}
                >
                  {number}
                </span>
                <span className={`${active ? "text-[var(--ink)]" : "text-[var(--muted)]"} hidden sm:inline`}>{label}</span>
              </li>
            );
          })}
        </ol>
      </div>
      <main id="contenido-principal" className="page-enter mx-auto w-full max-w-5xl flex-1 px-4 py-6 sm:px-6 sm:py-9 lg:px-8">
        {children}
      </main>
      <FooterNav />
    </div>
