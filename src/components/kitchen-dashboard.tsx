...
export function KitchenDashboard({ colegioId }: { colegioId: ColegioId }) {
  ...
  const tabButton = (value: Tab, label: string, desktop: boolean) => {
    const active = tab === value;
    const anchor = value === "entrega" ? { "data-tour": "panel-entrega-tab" } : {};
    return (
      <button
        type="button"
        role="tab"
        ...
        className={
          desktop
            ? `rounded-xl px-3.5 py-2.5 text-left text-sm font-extrabold transition-colors ${active ? "bg-[var(--coral-soft)] text-[var(--coral-dark)]" : "text-[var(--muted)] hover:bg-[var(--ink-soft)] hover:text-[var(--ink)]"}`
            : `rounded-lg px-4 text-sm font-extrabold transition-colors ${active ? "bg-[var(--paper)] text-[var(--ink)] shadow-sm" : "text-[var(--muted)]"}`
        }
  ...
  return (
    <PanelShell
      tabs={
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex min-h-11 rounded-xl bg-[oklch(94%_0.012_80)] p-1" role="tablist" aria-label="Vista operativa">
            {tabButton("preparacion", "Preparación", false)}
            {tabButton("entrega", "Entrega", false)}
          </div>
      ...
      sidebar={
        <>
          <div>
            <p className="mb-0.5 text-[0.65rem] font-extrabold uppercase tracking-[0.06em] text-[var(--muted)]">Casino Delicor</p>
            <p className="mb-0 text-sm font-extrabold">{colegio.name}</p>
          </div>
          <nav className="grid gap-1" role="tablist" aria-label="Vista operativa">
            {tabButton("preparacion", "Preparación", true)}
            {tabButton("entrega", "Entrega", true)}
          </nav>
          <Link href={`/casino/${otherColegio.id}`} className="btn-quiet justify-start px-3.5 text-xs">
            <Building2 size={14} /> Cambiar a {otherColegio.shortName}
          </Link>
        </>
      }
    >
